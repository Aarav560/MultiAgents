...
| `cd: <folder>: No such file or directory` | Your terminal isn't at the top of the repo (the prompt shows `~` or a subfolder), or you're on a branch without that folder. Run `cd "$(git rev-parse --show-toplevel)"`, then check `git branch --show-current`. |
